...
def application_Filter(msg):
    ...
    for p in preposition_list:
         if primary_Command == token[0] and  token[1] == p:
             print(msg_filter(msg, p, "null"))
             print('removed')
    
          
    
    #for c in filter_conjunctions:
    #    f_one = msg_filter(msg, c, " ")

    #f_two = msg_filter(f_one, primary_Command," ") 
    #f_Three = f_two.replace(" ", "")
    #if secondary_Command != " ":
    #    startin_word = keyWord_finder(secondary_Command[0], f_two, 2) 
    #    secondry_command_msg = array_string(len(f_two), startin_word,f_two)  
    #    f_Three = msg_filter(secondry_command_msg, " ")
    #    return f_Three
    #else:
    #    return f_Three
